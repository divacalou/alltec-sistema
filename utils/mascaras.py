...
# Importação opcional/segura de EntradaMascarada
try:
    from mascaras import EntradaMascarada
except ImportError:
    EntradaMascarada = None
...
class JanelaFormularioEPI(tk.Toplevel):
    ...
    def _montar_layout(self):
        ...
        campos = [
            ("nome_epi", "Nome do EPI*", None),
            ("numero_ca", "Número do CA", None),
            ("data_entrega", "Data de Entrega (dd/mm/aaaa)", "data"),
            ("quantidade", "Quantidade", None),
            ("validade_dias", "Validade (dias até troca)", None),
            ("observacoes", "Observações", None),
        ]

        linha = 1
        for chave, rotulo, mascara in campos:
            ...
            if mascara and EntradaMascarada:
                entry = EntradaMascarada(container, mascara=mascara, textvariable=var, width=28)
            else:
                entry = ttk.Entry(container, textvariable=var, width=28)

            entry.grid(row=linha, column=1, sticky="ew", padx=4, pady=6)
    ...
    def salvar(self):
        ...
        # Validação de data via EntradaMascarada (se disponível)
        entry_data = self.entradas.get("data_entrega")
        if EntradaMascarada and isinstance(entry_data, EntradaMascarada):
            if not entry_data.data_valida_ou_vazia():
                messagebox.showwarning("Data Inválida", "Informe uma data de entrega válida.", parent=self)
                return
...
class JanelaFormularioExame(tk.Toplevel):
    ...
    def _montar_layout(self):
        ...
        campos = [
            ("data_realizacao", "Data de Realização (dd/mm/aaaa)", "data"),
            ("periodicidade_meses", "Periodicidade (meses)", None),
            ("medico", "Médico Responsável", None),
            ("observacoes", "Observações", None),
        ]
    ...
    def salvar(self):
        ...
        # Validação de data via EntradaMascarada (se disponível)
        entry_data = self.entradas.get("data_realizacao")
        if EntradaMascarada and isinstance(entry_data, EntradaMascarada):
            if not entry_data.data_valida_ou_vazia():
                messagebox.showwarning("Data Inválida", "Informe uma data de realização válida.", parent=self)
                return
